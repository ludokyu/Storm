 ...
 */
class Zend_Form_Decorator_Datalist extends Zend_Form_Decorator_Abstract
{
    /**
    ...
    public function render($content)
    {
        ...
        $this->setOption("id","datalist-".$element->getName());
        $options   = $this->getOptions();

        foreach ($datalist as $data) {
            $datalisthtml = "<option value='$data'>";
        }
        ...
        if (!empty($tag)) {
            ...
            $datalisthtml = $decorator->render($datalisthtml);
        }

        switch ($placement) {
            case self::PREPEND:
                return $datalisthtml . $separator . $content;
            case self::APPEND:
            default:
                return $content . $separator . $datalisthtml;
        }
    }
}
